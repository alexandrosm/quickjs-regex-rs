...
pub enum Prefilter {
    ...
    AnchoredStart,
    /// Look for a single byte to find candidates
    SingleByte(u8),
    /// Look for any of these bytes to find candidates
    ByteSet(Vec<u8>),
    /// Look for a literal string using memmem
    Memmem(Vec<u8>),
    /// Look for any of these literal strings using Aho-Corasick
    AhoCorasick(Vec<Vec<u8>>),
}

/// Derive the best prefilter from static analysis.
pub fn derive_prefilter(info: &StaticInfo) -> Prefilter {
    // Anchored patterns only need to try position 0
    if info.anchored_start {
        return Prefilter::AnchoredStart;
    }

    // Required literals: pick the longest one for memmem
    if let Some(best) = info.required_literals.iter()
        .max_by_key(|s| s.len())
        .filter(|s| s.len() >= 2)
    {
        return Prefilter::Memmem(best.as_bytes().to_vec());
    }

    // Multiple possible literals (from alternation): use Aho-Corasick
    if info.possible_literals.len() >= 2 {
        let patterns: Vec<Vec<u8>> = info.possible_literals.iter()
            ...
            .collect();
        if patterns.len() >= 2 {
            return Prefilter::AhoCorasick(patterns);
        }
    }

    // Start bytes: use single byte or byte set
...
mod tests {
    ...
    fn test_prefilter_memmem() {
        ...
        // Should extract "hello" or "world" as memmem prefilter
        match pf {
            Prefilter::Memmem(bytes) => {
                let s = String::from_utf8(bytes).unwrap();
                assert!(s == "hello" || s == "world");
            }
            _ => panic!("expected Memmem, got {:?}", pf),
        }
    }
    ...
    fn test_prefilter_alternation() {
        ...
        let pf = derive_prefilter(&info);
        match pf {
            Prefilter::AhoCorasick(patterns) => {
                assert!(patterns.len() >= 2);
            }
            _ => panic!("expected AhoCorasick, got {:?}", pf),
        }
    }
    ...
    fn test_complex_prefilter() {
        ...
        match pf {
            Prefilter::AhoCorasick(patterns) => {
                ...
                let strs: Vec<String> = patterns.iter()
                    .map(|p| String::from_utf8(p.clone()).unwrap())
                    .collect();
                assert!(strs.contains(&"ASIA".to_string()));
                assert!(strs.contains(&"AKIA".to_string()));
            }
            other => panic!("expected AhoCorasick, got {:?}", other),
        }
    }
